...
const adminStats = [
  ...
  { label: "Выручка (месяц)", value: "₽186 400", icon: "Banknote", trend: "+12%" },
];

const coachStats = [
  { label: "Мои группы", value: "4", icon: "Users" },
  { label: "Тренировок в неделю", value: "18", icon: "CalendarDays" },
  { label: "Спортсменов", value: "34", icon: "UserCheck" },
  { label: "Ср. посещаемость", value: "87%", icon: "TrendingUp" },
];
  ...
  if (role === "admin") {
    return (
      <div className="p-8 space-y-8 animate-fade-in">
        <div>
          <p className="text-dim text-sm mb-1">Понедельник, 4 мая 2026</p>
          <h1 className="font-display text-3xl font-semibold">Обзор платформы</h1>
        </div>
        <div className="grid grid-cols-4 gap-4">
          {adminStats.map((s) => (
            <div key={s.label} className="stat-card">
              <div className="flex items-center justify-between">
                <Icon name={s.icon} size={18} fallback="Circle" className="text-dim" />
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="card-base p-6">
            <p className="text-dim text-xs mb-4 uppercase tracking-widest">Быстрые действия</p>
            <div className="space-y-2">
              {[
                ...
                <button
                  key={a.label}
                  className="w-full flex items-center gap-3 p-3 rounded-lg bg-secondary hover:bg-muted transition-colors text-sm text-left"
                >
                  <Icon name={a.icon} size={16} fallback="Circle" className="text-aqua" />
                  {a.label}
                </button>
              ))}
            </div>
  ...
  if (role === "coach") {
    return (
      <div className="p-8 space-y-8 animate-fade-in">
        <div>
          <p className="text-dim text-sm mb-1">Понедельник, 4 мая 2026</p>
          <h1 className="font-display text-3xl font-semibold">Кабинет тренера</h1>
        </div>
        <div className="grid grid-cols-4 gap-4">
          {coachStats.map((s) => (
            <div key={s.label} className="stat-card">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="card-base p-6">
            <p className="text-dim text-xs mb-4 uppercase tracking-widest">Тренировки сегодня</p>
            <div className="space-y-3">
              {upcomingTrainings.map((t) => (
                <div key={t.time} className="flex items-start gap-4 p-3 bg-secondary rounded-lg">
          ...
          <div className="card-base p-6">
            <p className="text-dim text-xs mb-4 uppercase tracking-widest">Посещаемость групп</p>
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-dim text-sm mb-1">Понедельник, 4 мая 2026</p>
          <h1 className="font-display text-3xl font-semibold">Привет, Алексей</h1>
        </div>
        <div className="card-base px-4 py-2 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-400" />
          <span className="text-xs text-dim">Абонемент активен · 18 дней</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="col-span-2 card-base p-6">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs text-dim">Цель: 20 000 м</span>
          </div>
          <p className="font-display text-4xl font-semibold text-aqua mt-1">
            {totalWeekMeters.toLocaleString("ru")}
            <span className="text-base text-dim font-normal ml-2">м</span>
          </p>
          <div className="progress-bar mt-4">
            <div className="progress-fill" style={{ width: `${(totalWeekMeters / 20000) * 100}%` }} />
          </div>
          <p className="text-xs text-dim mt-2">{Math.round((totalWeekMeters / 20000) * 100)}% от цели</p>
        </div>

        <div className="stat-card">
          <Icon name="Flame" size={18} fallback="Circle" className="text-orange-400" />
          <p className="font-display text-2xl font-semibold mt-2">7</p>
          <p className="text-dim text-xs">Тренировок подряд</p>
        </div>
        <div className="stat-card">
          <Icon name="Timer" size={18} fallback="Circle" className="text-aqua" />
          <p className="font-display text-2xl font-semibold mt-2">12.4</p>
          <p className="text-dim text-xs">Ср. скорость (сек/25м)</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
              ...
              return (
                <div key={d} className="flex flex-col items-center gap-2 flex-1">
                  <div className="w-full relative flex items-end" style={{ height: "100px" }}>
                    {weekMeters[i] > 0 && (
                      <div
                        className="absolute -top-5 w-full text-center text-xs text-dim"
                      >
                        {(weekMeters[i] / 1000).toFixed(1)}к
                      </div>
                    )}
                    <div
                      className="w-full rounded-t transition-all duration-700"
        ...
        {/* Стили плавания */}
        <div className="card-base p-6">
          <p className="text-dim text-xs mb-4 uppercase tracking-widest">По стилям</p>
          <div className="space-y-3">
            {styles.map((s) => (
      ...
      <div className="card-base p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="text-xs text-aqua hover:opacity-80 transition-opacity">Все →</button>
        </div>
        <div className="grid grid-cols-3 gap-3">
          {upcomingTrainings.map((t) => (
            <div key={t.time} className="p-4 bg-secondary rounded-lg border border-border">
              <div className="flex items-center justify-between mb-2">
                <span className="font-display text-aqua text-sm font-semibold">{t.time}</span>
                <span className="text-xs text-dim">{t.lane}</span>
              </div>
              <p className="text-sm font-medium">{t.title}</p>
              <p className="text-dim text-xs mt-1">{t.coach}</p>
            </div>
          ))}
        </div>
      </div>
